...
/// Validates raw signatures with an RSA signature algorithm that is
/// not supported directly by C2PA. (Some RFC 3161 time stamp providers
/// issue these signatures, which is why it's supported here.)
...
mod tests {
    ...
    // 512 zero bytes: accepted by rsa::pkcs1v15::Signature::try_from (only empty
    // slices are rejected); content correctness is checked at verify time,
    // after our guard fires.
